...
- Jw_cad で描いた枠を外部変形で送り `profile_frame_from_job` で保存することもできる（値の欄は空、会社名などは欲しい大きさで描く）
- ロゴなどのビットマップは `profile_set_logo` / `image_trace`（OpenCV の輪郭追跡、`pip install jwmcp[raster]`）で線データに変換して枠に入れる
